...
// Add file transport specifically for collection submissions
if (process.env.LOGGER_SUBMISSIONS_LOG_FILE) {
  logger.add(
    new transports.File({
      filename: process.env.LOGGER_SUBMISSIONS_LOG_FILE,
      level: 'info',
      decolorize: true,
      format: combine(
        baseFormat,
        // Add a filter to only include collection submission logs
        format((info) => {
          // Only include logs with event: "collection_submission"
          if (
            info.metadata &&
            info.metadata.event === 'collection_submission'
          ) {
            return info
          }
          return false // Skip other logs
        })(),
        decolorize(),
        json()
      )
